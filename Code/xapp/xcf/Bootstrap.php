...
class XCF_Bootstrap extends XIDE_Bootstrap
{
	...
	private static function createServerInstance(
		...
		$driverScopes = array(
			...
			XIDE_Scope::optionFactory(XIDE_Scope::APP)
		);
		...
		$XAPP_NODEJS_CONFIG_PATH = realpath(
				$xcvRoot
			) . DIRECTORY_SEPARATOR . 'services-' . (XApp_Service_Entry_Utils::isDebug(
			) ? 'debug' : 'release') . '.json';
				...
				XApp_Service::factoryEx(
					...
					array(
						...
						XApp_Service::MANAGED_CLASS_OPTIONS => array(
							...
							XIDE_NodeJS_Service_Manager::FORCE_HOST => XApp_Service_Entry_Utils::isDesktopApp(
							) ? '127.0.0.1' : $node_server_host_ip,
	...
	public function setup()
	{
		...
		//$xappResourceRenderer->registerRelative('DOJOPACKAGES', "{}");
		/****
