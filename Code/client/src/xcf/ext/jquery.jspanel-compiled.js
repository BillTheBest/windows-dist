...
var jsPanel = {
    ...
    position: function position(elmt, options) {
        ...
        if (typeof options === 'string') {
            ...
            var rxautopos = /DOWN|UP|RIGHT|LEFT/;
            var rxoffset = /[+-]?\d+\.?\d*%?/g;
            var posValue = options.match(rxpos);
            var autoposValue = options.match(rxautopos);
            var offsetValue = options.match(rxoffset);
        ...
        if (typeof option.offsetX === 'string' && option.offsetX.slice(-1) === '%') {
            ...
            option.offsetX = parseInt(option.offsetX());
        }
        if (typeof option.offsetY === 'string' && option.offsetY.slice(-1) === '%') {
            ...
            option.offsetY = parseInt(option.offsetY());
        }
        ...
        if (option.autoposition) {
            var newClass,
                ...
                allNewClass = [],
                i,
                spacing = jsPanel.autopositionSpacing;
            ...
            if (option.autoposition === 'DOWN') {
                ...
                for (i = 0; i < allNewClass.length - 1; i++) {
    ...
    color: function color(val) {
        ...
        var RGBAPattern = /^rgba?\(([0-9]{1,3}),([0-9]{1,3}),([0-9]{1,3}),?(0|1|0\.[0-9]{1,2}|\.[0-9]{1,2})?\)$/gi; // matches rgb/rgba color values, whitespace allowed
        var HSLAPattern = /^hsla?\(([0-9]{1,3}),([0-9]{1,3}\%),([0-9]{1,3}\%),?(0|1|0\.[0-9]{1,2}|\.[0-9]{1,2})?\)$/gi;
        var namedColors = {
            aliceblue: 'f0f8ff',
            antiquewhite: 'faebd7',
            aqua: '0ff',
    ...
    calcColors: function calcColors(primaryColor) {
        var primeColor = this.color(primaryColor),
            secondaryColor = this.lighten(primaryColor, .81),
            fontColorHeader;
        // calculate perceived brightness
        var pBrightness = this.perceivedBrightness(primaryColor);
        pBrightness <= this.pbTreshold ? fontColorHeader = '#ffffff' : fontColorHeader = '#000000';
        return [primeColor.rgb.css, secondaryColor, fontColorHeader];
    },

    /* ----------------------------------------------------------------------------------------------- */

    /* modal specific functions ---------------------------------------------------------------------- */
    setModalCSS: function setModalCSS(panel) {
        panel.addClass('jsPanel-modal').css('z-index', 10000);
        $(panel.header, panel.header.title, panel.footer).css('cursor', 'default');
        $('.jsPanel-title', panel).css('cursor', 'inherit');
    },
    insertModalBackdrop: function insertModalBackdrop() {
        if ($('.jsPanel-modal-backdrop').length < 1) {
            var backdrop = '<div class="jsPanel-modal-backdrop" style="height:' + $(document).outerHeight() + 'px;"></div>';
    ...
    setHintCSS: function setHintCSS(panel) {
        panel.addClass('jsPanel-hint').css('z-index', 10000);
        $(panel.header, panel.header.title, panel.footer).css('cursor', 'default');
        $('.jsPanel-title', panel).css('cursor', 'inherit');
    },
    ...
    addConnector: function addConnector(panel) {
        var bgColor = panel.option.paneltype.connectorBG || null,
            ...
            topConns = /top|topleft|topright|lefttopcorner|righttopcorner|leftbottom|rightbottom/,
            bottomConns = /bottom|bottomleft|bottomright|leftbottomcorner|rightbottomcorner/,
            leftrightTopConns = /lefttop|righttop/,
            leftrightConns = /left|right/;

        var calcConnectorBgTop = function calcConnectorBgTop(connector) {
            if (connector.match(topConns)) {
        ...
        var calcConnectorBgBottom = function calcConnectorBgBottom(connector) {
            if (connector.match(bottomConns)) {
        ...
        var calcConnectorBgLeftRightTop = function calcConnectorBgLeftRightTop(connector) {
            if (connector.match(leftrightTopConns)) {
                if (!panel.option.headerRemove) {
                    return bgColor_panel;
                }
                return bgColor_content;
        ...
        var calcConnectorBgLeftRight = function calcConnectorBgLeftRight(connector) {
            if (connector.match(leftrightConns)) {
                if (parseFloat(panel.option.contentSize.height) > 0) {
                    return bgColor_content;
                }
                if (!panel.option.headerRemove) {
        ...
        if (panel.hasClass('jsPanel-tooltip-top')) {
            ...
            console.log(calcConnectorBgBottom('rightbottomcorner'));
        } else if (panel.hasClass('jsPanel-tooltip-leftbottomcorner')) {
            // tooltip leftbottomcorner
    ...
    setTooltipCSS: function setTooltipCSS(panel) {
        $(panel.header.title, panel.footer).css('cursor', 'inherit');
        panel.css('overflow', 'visible');
        panel.addClass('jsPanel-tooltip');
    },
    ...
    iframe: function iframe(panel) {
        ...
        //iframe size
        if (panel.option.contentSize.width !== "auto" && !poi.width) {
            iFrame.css("width", "100%");
        } else {
            iFrame.prop("width", poi.width);
        }
        if (panel.option.contentSize.height !== "auto" && !poi.height) {
            iFrame.css("height", "100%");
        } else {
            iFrame.prop("height", poi.height);
        }
        //iframe name
        if (poi.name) {
...
(function ($) {
    ...
    $.jsPanel = function (config) {
        ...
        if (jsP.option.paneltype.tooltip) {
            ...
            // if tooltip && trigger already has a tooltip exit jsPanel()
        ...
        var passedTheme = jsP.option.theme.toLowerCase().replace(/ /g, ""),
            theme = [],
            bs,
            bstheme;
        ...
        if (!navigator.appVersion.includes('Trident')) {
            var eventjsPanelLoaded = new CustomEvent('jspanelloaded', { detail: jsP.attr('id') }),
                eventjsPanelStatusChange = new CustomEvent('jspanelstatuschange', { detail: jsP.attr('id') }),
                eventjsPanelBeforeClose = new CustomEvent('jspanelbeforeclose', { detail: jsP.attr('id') }),
                eventjsPanelClosed = new CustomEvent('jspanelclosed', { detail: jsP.attr('id') }),
                eventjsPanelBeforeMaximize = new CustomEvent('jspanelbeforemaximize', { detail: jsP.attr('id') }),
                eventjsPanelMaximized = new CustomEvent('jspanelmaximized', { detail: jsP.attr('id') }),
                eventjsPanelBeforeNormalize = new CustomEvent('jspanelbeforenormalize', { detail: jsP.attr('id') }),
                eventjsPanelNormalized = new CustomEvent('jspanelnormalized', { detail: jsP.attr('id') }),
                eventjsPanelBeforeMinimize = new CustomEvent('jspanelbeforeminimize', { detail: jsP.attr('id') }),
                eventjsPanelMinimized = new CustomEvent('jspanelminimized', { detail: jsP.attr('id') }),
                eventjsPanelSmallified = new CustomEvent('jspanelsmallified', { detail: jsP.attr('id') }),
                eventjsPanelSmallifiedMax = new CustomEvent('jspanelsmallifiedmax', { detail: jsP.attr('id') });
        } else {
            // if browser is IE11
            var eventjsPanelLoaded = document.createEvent('CustomEvent'),
                eventjsPanelStatusChange = document.createEvent('CustomEvent'),
                eventjsPanelBeforeClose = document.createEvent('CustomEvent'),
                eventjsPanelClosed = document.createEvent('CustomEvent'),
                eventjsPanelBeforeMaximize = document.createEvent('CustomEvent'),
                eventjsPanelMaximized = document.createEvent('CustomEvent'),
                eventjsPanelBeforeNormalize = document.createEvent('CustomEvent'),
                eventjsPanelNormalized = document.createEvent('CustomEvent'),
                eventjsPanelBeforeMinimize = document.createEvent('CustomEvent'),
                eventjsPanelMinimized = document.createEvent('CustomEvent'),
                eventjsPanelSmallified = document.createEvent('CustomEvent'),
                eventjsPanelSmallifiedMax = document.createEvent('CustomEvent');
            eventjsPanelLoaded.initCustomEvent('jspanelloaded', true, true, jsP.attr('id'));
            eventjsPanelStatusChange.initCustomEvent('jspanelstatuschange', true, true, jsP.attr('id'));
            eventjsPanelBeforeClose.initCustomEvent('jspanelbeforeclose', true, true, jsP.attr('id'));
            eventjsPanelClosed.initCustomEvent('jspanelclosed', true, true, jsP.attr('id'));
            eventjsPanelBeforeMaximize.initCustomEvent('jspanelbeforemaximize', true, true, jsP.attr('id'));
            eventjsPanelMaximized.initCustomEvent('jspanelmaximized', true, true, jsP.attr('id'));
            eventjsPanelBeforeNormalize.initCustomEvent('jspanelbeforenormalize', true, true, jsP.attr('id'));
            eventjsPanelNormalized.initCustomEvent('jspanelnormalized', true, true, jsP.attr('id'));
            eventjsPanelBeforeMinimize.initCustomEvent('jspanelbeforeminimize', true, true, jsP.attr('id'));
            eventjsPanelMinimized.initCustomEvent('jspanelminimized', true, true, jsP.attr('id'));
            eventjsPanelSmallified.initCustomEvent('jspanelsmallified', true, true, jsP.attr('id'));
            eventjsPanelSmallifiedMax.initCustomEvent('jspanelsmallifiedmax', true, true, jsP.attr('id'));
        }
        ...
        jsP.close = function () {
            document.dispatchEvent(eventjsPanelBeforeClose);
            if ($.isFunction(jsP.option.onbeforeclose)) {
                var close = jsP.option.onbeforeclose.call(jsP, jsP);
                if (close === false) {
                    return jsP;
                }
            }
            jsP.remove();
            document.dispatchEvent(eventjsPanelClosed);
            document.dispatchEvent(eventjsPanelStatusChange);
            if (jsP.option.paneltype === 'modal') {
                $('.jsPanel-modal-backdrop').remove();
            }
            if (jsP.option.paneltype.tooltip) {
                $(trigger).removeClass('hasTooltip');
            }
        ...
        //jsP.headerControl = (ctrl, action = 'enable') => jsPanel.headerControl(jsP, ctrl, action);
        jsP.headerControl = function (ctrl) {
            ...
            if (ctrl) {
                ...
                ['close', 'maximize', 'minimize', 'normalize', 'smallify'].forEach(function (value, index, array) {
        ...
        jsP.normalize = function () {
            ...
            document.dispatchEvent(eventjsPanelBeforeNormalize);
            if ($.isFunction(jsP.option.onbeforenormalize)) {
                var normalize = jsP.option.onbeforenormalize.call(jsP, jsP);
                if (normalize === false) {
                    return jsP;
                }
            }
            ...
            document.dispatchEvent(eventjsPanelNormalized);
            document.dispatchEvent(eventjsPanelStatusChange);
        ...
        jsP.maximize = function () {
            ...
            document.dispatchEvent(eventjsPanelBeforeMaximize);
            if ($.isFunction(jsP.option.onbeforemaximize)) {
                var maximize = jsP.option.onbeforemaximize.call(jsP, jsP);
                if (maximize === false) {
                    return jsP;
                }
            }
            ...
            // remove replacement
            $('#' + jsP.prop('id') + '-min').remove();
            document.dispatchEvent(eventjsPanelMaximized);
            document.dispatchEvent(eventjsPanelStatusChange);
        ...
        jsP.minimize = function () {
            ...
            document.dispatchEvent(eventjsPanelBeforeMinimize);
            if ($.isFunction(jsP.option.onbeforeminimize)) {
                var minimize = jsP.option.onbeforeminimize.call(jsP, jsP);
                if (minimize === false) {
                    return jsP;
                }
            }
            ...
            document.dispatchEvent(eventjsPanelMinimized);
            document.dispatchEvent(eventjsPanelStatusChange);
            ...
            if (jsP.data('status') === "normalized" || jsP.data('status') === "maximized") {
                if (jsP.data('status') !== "smallified" && jsP.data('status') !== "smallifiedMax") {
                    ...
                    if (jsP.data('status') === 'maximized') {
                        ...
                        document.dispatchEvent(eventjsPanelSmallifiedMax);
                    } else {
                        jsPanel.hideControls(".jsPanel-btn-normalize, .jsPanel-btn-smallify", jsP);
                        jsP.data('status', 'smallified');
                        document.dispatchEvent(eventjsPanelSmallified);
                    }
                    document.dispatchEvent(eventjsPanelStatusChange);
                }
        ...
        /* option.headerRemove,
         option.headerControls (buttons in header right) ------------------------------------- */
        if (!jsP.option.headerRemove) {
            if (jsP.option.headerControls.buttons === 'closeonly') {
                $(".jsPanel-btn-minimize, .jsPanel-btn-normalize, .jsPanel-btn-maximize, .jsPanel-btn-smallify, .jsPanel-btn-smallifyrev", jsP.header.headerbar).remove();
            } else if (jsP.option.headerControls.buttons === 'none') {
                $(jsP.header.controls).remove();
            }
            // disable controls individually
            ["close", "maximize", "minimize", "normalize", "smallify"].forEach(function (ctrl) {
                if (jsP.option.headerControls[ctrl]) {
        ...
        /* option.paneltype ----------------------------------------------------------------------------------------- */
        if (jsP.option.paneltype === "modal") {
            jsPanel.setModalCSS(jsP);
            jsPanel.insertModalBackdrop();
            /*
             To generate something like a modal jsPanel without actually using option.paneltype = 'modal':
             jsPanel.insertModalBackdrop();
             $.jsPanel({
             onminimized: function(){
             // without this a minimized panel could not be restored via controls
             $('#jsPanel-replacement-container').css('z-index', '10000');
             }
             }).addClass('jsPanel-modal').css('z-index', '10000');
             // after restoring such a panel the modal backdrop has to be removed manually
             */
        } else if (jsP.option.paneltype === 'hint') {
            jsPanel.setHintCSS(jsP);
        } else if (jsP.option.paneltype.tooltip) {
            trigger = jsPanel.setTrigger(jsP.option.position); // elmt triggering the tooltip
            jsPanel.setTooltipCSS(jsP);
        ...
        document.dispatchEvent(eventjsPanelLoaded);
        ...
        if (!bstheme) {
            if (jsPanel.themes.includes(theme[0])) {
                ...
                var colors = jsPanel.calcColors(theme[0]); // colors: [primeColor, secondaryColor, fontColorHeader]
                jsP.css('background-color', colors[0]);
                $('.jsPanel-hdr *', jsP).css({ color: colors[2] });
                if (jsP.option.headerToolbar) {
                    jsP.header.toolbar.css({ boxShadow: '0 0 1px ' + colors[2] + ' inset', width: 'calc(100% + 4px)', marginLeft: '-2px' });
                } else {
                    jsP.content.css({ borderTop: '1px solid ' + colors[2] });
                }
                if (fill === 'filled') {
                    jsP.content.css({ 'background-color': colors[0], color: colors[2] });
                } else if (fill === 'filledlight') {
            ...
            $('*', jsP.header).css('color', bsColors[2]);
            if (jsP.option.headerToolbar) {
                jsP.header.toolbar.css({ boxShadow: '0 0 1px ' + bsColors[2] + ' inset', width: 'calc(100% + 4px)', marginLeft: '-2px' });
            } else {
                jsP.content.css({ borderTop: '1px solid ' + bsColors[2] });
            }
            if (fill === 'filled') {
                jsP.content.css({ backgroundColor: pColor, color: bsColors[2] });
            }
            if (fill === 'filledlight') {
        ...
        if ($.isPlainObject(jsP.option.contentAjax)) {
            jsPanel.ajax(jsP);
        }
        ...
        jsP.css({
            // neccessary if title text exceeds content width & correction for panel padding
        ...
        if (typeof jsP.option.show === "string") {
            jsP.addClass(jsP.option.show).css('opacity', 1); //extra call to jQuery.css() needed for EDGE
        }
        ...
        // handler to move panel to foreground on mouseup
        jsP[0].addEventListener('mouseup', function (e) {
        ...
        if (typeof jsP.option.setstatus === 'string') {
            if (jsP.option.setstatus === 'maximize smallify') {
                jsP.maximize().smallify();
            } else {
                jsP[jsP.option.setstatus]();
            }
        }
    ...
    /* the next two event handlers take care of repositioning autopositioned jsPanels after one was removed */
    document.addEventListener('jspanelbeforeclose', function (e) {
        var regex = /left-top|center-top|right-top|left-center|center|right-center|left-bottom|center-bottom|right-bottom/;
        var panelclass = document.getElementById(e.detail).className;
        var parent = $('#' + e.detail).parent();
        var match = panelclass.match(regex);
        if (match) {
    ...
    document.addEventListener('jspanelclosed', function () {
        if (jsPanel.lastbeforeclose) {
            var container = jsPanel.lastbeforeclose.parent;
            var panels = $('.' + jsPanel.lastbeforeclose.class, container);
            var pos = jsPanel.lastbeforeclose.class;
        }
        ...
        if (panels) {
            ...
            panels.each(function (index, elmt) {
                ...
                var oX = elmt.getAttribute('data-offsetx');
                var oY = elmt.getAttribute('data-offsety');
